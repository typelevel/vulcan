...
/** Annotation which can be used to include the record alias in derived schemas.
  ...
  *   - Annotate a type for enum alias when using [[deriveEnum]].<br>
  *   - Annotate a type for fixed alias when using [[deriveFixed]].<br>
  *   - Annotate a `case class` for record alias when using `Codec.derive` from the generic
  *     module.<br>
  *   - Annotate a `case class` parameter for record field alias when using `Codec.derive` from the
  *     generic module.
  */
