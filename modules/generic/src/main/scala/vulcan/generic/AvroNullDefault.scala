...
/** Annotation which can be used to enable/disable explicit default null values for nullable fields
  ...
  *   - Annotate a `case class` to enable/disable explicit default null values in the schema for all
  *     nullable fields when using `Codec.derive` from the generic module.<br>
  *   - Annotate a `case class` parameter to enable/disable explicit default null value in the
  *     schema for this specific nullable field when using `Codec.derive` from the generic module.
  *
