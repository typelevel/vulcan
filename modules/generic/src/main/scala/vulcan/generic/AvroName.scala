...
/** Annotation which can be used to alter the record name in derived schemas.
  ...
  *   - Annotate a type for enum name when using [[deriveEnum]].<br>
  *   - Annotate a type for fixed name when using [[deriveFixed]].<br>
  *   - Annotate a `case class` for record name when using `Codec.derive` from the generic
  *     module.<br>
  *   - Annotate a `case class` parameter for record field name when using `Codec.derive` from the
  *     generic module.
  */
