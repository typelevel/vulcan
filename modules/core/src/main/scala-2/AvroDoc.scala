...
/** Annotation which can be used to include documentation in derived schemas.
  ...
  *   - Annotate a type for enum documentation when using [[Codec.deriveEnum]].<br>
  *   - Annotate a type for fixed documentation when using [[Codec.deriveFixed]].<br>
  *   - Annotate a `case class` for record documentation when using `Codec.derive` from the generic
  *     module.<br>
