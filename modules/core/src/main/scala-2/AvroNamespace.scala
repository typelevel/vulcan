...
/** Annotation which can be used to set the namespace in derived schemas.
  ...
  *   - Annotate a type for enum namespace when using [[Codec.deriveEnum]].<br>
  *   - Annotate a type for fixed namespace when using [[Codec.deriveFixed]].<br>
  *   - Annotate a `case class` for record namespace when using `Codec.derive` from the generic
  *     module.
  */
